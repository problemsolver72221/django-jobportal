...
def _validate_password_similarity(password, username="", email=""):
    errors = []
...
class EmployerSignUpForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        base_class = "w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all hover:border-blue-400"
        ...
        for name, field in self.fields.items():
            if isinstance(field.widget, forms.FileInput):
                field.widget.attrs.setdefault(
                    "class", "w-full px-2 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all hover:border-blue-400"
                )
                continue
...
class JobSeekerSignUpForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        base_class = "w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all group-hover:border-purple-400"
        textarea_class = f"{base_class} resize-y"
        placeholders = {
            "username": "Choose a username",
        ...
        for name, field in self.fields.items():
            if isinstance(field.widget, forms.FileInput):
                field.widget.attrs.setdefault(
                    "class", "w-full px-2 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all group-hover:border-purple-400"
                )
                continue
            base = textarea_class if isinstance(field.widget, forms.Textarea) else base_class
            field.widget.attrs.setdefault("class", base)
